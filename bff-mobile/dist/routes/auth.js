...
function setupAuthRoutes(app, backendUrl) {
    ...
    app.post("/api/mobile/auth/login", async (req, res) => {
        try {
            ...
            console.log("[BFF] Backend login response:", JSON.stringify(response.data, null, 2));
            // eslint-disable-next-line no-console
    ...
    app.post("/api/mobile/auth/register", async (req, res) => {
        try {
            console.log("[BFF-MOBILE] Received registration request:", JSON.stringify(req.body, null, 2));
            ...
            const response = await axios_1.default.post(`${backendUrl}/api/usuarios/auth/register`, backendPayload);
            console.log("[BFF-MOBILE] Backend register SUCCESS:", JSON.stringify(response.data, null, 2));
    ...
    // Verify Email
    app.post("/api/mobile/auth/verify-email", async (req, res) => {
        try {
    ...
    // Resend Verification
    app.post("/api/mobile/auth/resend-verification", async (req, res) => {
        try {
            const { email } = req.body;
            if (!email) {
                res.status(400).json({
