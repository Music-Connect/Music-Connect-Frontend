...
function setupPropostasRoutes(app, backendUrl) {
    ...
    app.get("/api/mobile/propostas/recebidas", async (req, res) => {
        try {
    ...
    app.get("/api/mobile/propostas/enviadas", async (req, res) => {
        try {
    ...
    app.get("/api/mobile/propostas/me", async (req, res) => {
        try {
            // Forward Authorization header to backend
            const authorization = req.headers.authorization || "";
            // eslint-disable-next-line no-console
            console.log("[BFF] GET /propostas/me - Authorization:", authorization ? `${authorization.substring(0, 30)}...` : "NONE");
            ...
            const response = await axios_1.default.get(`${backendUrl}/api/propostas/minhas`, {
                headers: {
                    ...(authorization && { Authorization: authorization }),
                },
            });
            // eslint-disable-next-line no-console
            console.log("[BFF] Backend /propostas/minhas response:", response.data.success);
            // Map backend fields to mobile-expected format
            const rawPropostas = response.data.data?.propostas || response.data.data || [];
            const propostas = (Array.isArray(rawPropostas) ? rawPropostas : []).map((p) => ({
                ...p,
    ...
    app.get("/api/mobile/propostas/:id", async (req, res, next) => {
        const { id } = req.params;
        ...
        try {
            const response = await axios_1.default.get(`${backendUrl}/api/propostas/${idStr}`);
    ...
    // Create proposal
    app.post("/api/mobile/propostas", async (req, res) => {
        try {
            console.log("[BFF-MOBILE] POST /api/mobile/propostas - Received data:", JSON.stringify(req.body, null, 2));
            const authorization = req.headers.authorization || "";
            console.log("[BFF-MOBILE] Authorization header:", authorization ? `${authorization.substring(0, 50)}...` : "MISSING");
            if (!authorization) {
            ...
            console.log("[BFF-MOBILE] Fetching current user from:", `${backendUrl}/api/usuarios/me`);
            let userResponse;
            try {
                userResponse = await axios_1.default.get(`${backendUrl}/api/usuarios/me`, {
                    headers: { Authorization: authorization },
                });
            }
            catch (userError) {
                console.error("[BFF-MOBILE] Error fetching current user:", userError);
                if (axios_1.default.isAxiosError(userError) && userError.response) {
                    console.error("[BFF-MOBILE] Backend error response:", userError.response.data);
                    res.status(userError.response.status).json({
                        success: false,
                        error: userError.response.data.error || "Erro ao identificar usuário",
                    });
                }
                else {
                    res.status(500).json({
            ...
            console.log("[BFF-MOBILE] User response structure:", JSON.stringify(userResponse.data, null, 2));
            // The backend returns { success: true, data: { ...user } }
            const currentUser = userResponse.data.data;
            console.log("[BFF-MOBILE] Current user:", currentUser?.id_usuario, currentUser?.usuario);
            if (!currentUser || !currentUser.id_usuario) {
                console.error("[BFF-MOBILE] Could not get current user. Response:", userResponse.data);
                res.status(401).json({
                    success: false,
                    error: "Não foi possível identificar o usuário",
                });
            ...
            const { id_artista, titulo, descricao, local, endereco_completo, tipo_evento, duracao_horas, publico_esperado, equipamento_incluso, nome_responsavel, telefone_contato, observacoes, data, hora, valor, } = req.body;
            ...
            console.log("[BFF-MOBILE] Sending to backend:", JSON.stringify(backendPayload, null, 2));
            const response = await axios_1.default.post(`${backendUrl}/api/propostas`, backendPayload, {
                headers: { Authorization: authorization },
            });
            ...
            if (axios_1.default.isAxiosError(error)) {
                if (error.response) {
                    console.error("[BFF-MOBILE] Backend error response:", JSON.stringify(error.response.data, null, 2));
                    res.status(error.response.status).json({
                        success: false,
                    ...
                    res.status(503).json({
                        success: false,
                        error: "Backend não está respondendo. Verifique se o serviço está rodando.",
                    });
                }
                else {
                ...
                console.error("[BFF-MOBILE] Unknown error:", error instanceof Error ? error.message : error);
                res.status(500).json({
                    success: false,
    ...
    app.put("/api/mobile/propostas/:id/status", async (req, res) => {
        try {
            const { id } = req.params;
            const authorization = req.headers.authorization || "";
    ...
    app.get("/api/mobile/propostas/recomendacoes", async (req, res) => {
        try {
            const authorization = req.headers.authorization || "";
            const { generos, local, tipo_evento, limit = "20", offset = "0", } = req.query;
            ...
            if (authorization && authorization.length > 0) {
                ...
                try {
                    const userResponse = await axios_1.default.get(`${backendUrl}/api/usuarios/me`, {
                        headers: { Authorization: authorization },
                    });
                    ...
                    console.log("[BFF] Could not authenticate user, showing public feed");
            ...
            try {
                ...
                let userGenres = [];
                if (currentUser?.tipo_usuario === "artista" &&
                    currentUser?.id_usuario) {
                    try {
                        const artistResponse = await axios_1.default.get(`${backendUrl}/api/artistas/${currentUser.id_usuario}`);
                        userGenres = artistResponse.data.data?.generos || [];
                    }
                    catch (error) {
                        console.log("[BFF] Could not fetch artist genres, continuing without filters");
                    }
                }
                ...
                const proposalsResponse = await axios_1.default.get(`${backendUrl}/api/propostas`, {
                ...
                console.error("[BFF] Error fetching proposals from backend:", {
                    ...
                    responseStatus: axios_1.default.isAxiosError(error)
                        ? error.response?.status
                        : "N/A",
                    responseData: axios_1.default.isAxiosError(error)
                        ? error.response?.data
                        : "N/A",
                    requestUrl: axios_1.default.isAxiosError(error) ? error.config?.url : "N/A",
                    requestHeaders: axios_1.default.isAxiosError(error)
            ...
            let filteredProposals = allProposals.filter((p) => {
                // Only show pending proposals
                if (p.status !== "pendente")
                    return false;
                // Don't show user's own proposals (if authenticated)
                if (currentUser) {
                    if (p.id_contratante === currentUser.id_usuario)
                        return false;
                    if (p.id_artista === currentUser.id_usuario)
                        return false;
            ...
            if (local && typeof local === "string") {
                ...
                filteredProposals = filteredProposals.filter((p) => {
                    const proposalLocal = (p.local_evento ||
                        p.local ||
                        "").toLowerCase();
            ...
            const paginatedProposals = filteredProposals.slice(offsetNum, offsetNum + limitNum);
            // Format proposals for mobile
            const formattedProposals = paginatedProposals.map((p) => ({
